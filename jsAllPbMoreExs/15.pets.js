function pets (input)
{
  ...
  let foodPerDayInGrTurtle = Number(input[4]);

  let dogEat = days * foodPerDayDog;
  let catEat = days * foodPerDayCat;
  let turtleEat =(days *foodPerDayInGrTurtle)/1000;

  let totalEatFood = dogEat + catEat + turtleEat;

  if (foodInKg > totalEatFood){
  console.log(`${Math.floor(foodInKg - totalEatFood)} kilos of food left.`);
  }
  else {
  console.log(`${Math.ceil(totalEatFood -foodInKg)} more kilos of food are needed.`);
  }
